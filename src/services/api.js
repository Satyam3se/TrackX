// Resolve API base URL: default to same-origin relative '/api/v1'
// If VITE_API_BASE_URL specifies localhost:8000 or :8000 from local django templates,
// automatically fall back to relative '/api/v1' to work directly with the server.
function getBaseUrl() {
...
async function apiFetch(path, options = {}) {
  ...
  try {
    ...
    if (BASE !== '/api/v1') {
      try {
        return await fetch(`/api/v1${path}`, options);
      } catch {
        throw err;
      }
    }
...
export async function addBlacklistedVehicle(data) {
  const res = await apiFetch('/blacklisted-vehicles/', {
    method: 'POST',
...
export async function deleteBlacklistedVehicle(id) {
  const res = await apiFetch(`/blacklisted-vehicles/${id}/`, {
    method: 'DELETE',
  });
...
export async function createVideoFeed(cameraId, title, file) {
  ...
  const res = await apiFetch('/video-feeds/', { method: 'POST', body: formData });
  if (!res.ok) {
    let detail = `HTTP ${res.status}`;
    try {
      const body = await res.json();
...
export async function processVideoFeed(feedId, sampleRate = 5) {
  const res = await apiFetch(`/video-feeds/${feedId}/process_video/`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
